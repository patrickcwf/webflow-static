...
async function convertFromCloudFrontRequestEvent(event) {
    ...
    return {
        ...
        cookies: cfHeaders.cookie?.reduce((acc, cur) => {
            const { key = "", value } = cur;
            acc[key] = value;
            return acc;
        }, {}) ?? {},
...
function convertToCloudfrontHeaders(headers, directResponse) {
...
async function convertToCloudFrontRequestResult(result, originalRequest) {
    if (result.type === "middleware") {
        ...
        let customOrigin = origin?.custom;
