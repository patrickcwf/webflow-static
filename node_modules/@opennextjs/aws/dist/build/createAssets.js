...
export function createCacheAssets(options) {
    ...
    const isFileSkipped = (relativePath) => relativePath.endsWith(".js") ||
    ...
    sourceDirs.forEach((sourceDir) => {
        buildHelper.traverseFiles(sourceDir, ({ relativePath }) => !isFileSkipped(relativePath), ({ absolutePath, relativePath }) => {
    ...
    if (!options.config.dangerous?.disableTagCache) {
        ...
        sourceDirs.forEach((sourceDir) => {
            buildHelper.traverseFiles(sourceDir, ({ absolutePath, relativePath }) => absolutePath.endsWith(".meta") && !isFileSkipped(relativePath), ({ absolutePath, relativePath }) => {
                const fileContent = fs.readFileSync(absolutePath, "utf8");
                const fileData = JSON.parse(fileContent);
