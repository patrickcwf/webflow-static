...
import { isEdgeRuntime } from "../helper.js";
import { copyOpenNextConfig, esbuildAsync } from "../helper.js";
export async function buildEdgeBundle({ middlewareInfo, entrypoint, outfile, options, defaultConverter, overrides, additionalInject, includeCache, additionalExternals, onlyBuildOnce, name, additionalPlugins: additionalPluginsFn, }) {
...
export async function generateEdgeBundle(name, options, fnOptions, additionalPlugins = () => []) {
    ...
    // Find functions
    const functions = Object.values(middlewareManifest.functions).filter((fn) => fnOptions.routes.includes(fn.name));
 ...
 */
export function copyMiddlewareResources(options, middlewareInfo, destDir) {
    mkdirSync(path.join(destDir, "wasm"), { recursive: true });
    for (const file of middlewareInfo?.wasm ?? []) {
        fs.copyFileSync(path.join(options.appBuildOutputPath, ".next", file.filePath), path.join(destDir, `wasm/${file.name}.wasm`));
    }
    mkdirSync(path.join(destDir, "assets"), { recursive: true });
    for (const file of middlewareInfo?.assets ?? []) {
        fs.copyFileSync(path.join(options.appBuildOutputPath, ".next", file.filePath), path.join(destDir, `assets/${file.name}`));
    }
}
