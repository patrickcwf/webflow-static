...
fix:
  '($STORE_OR_CACHE.isOnDemandRevalidate && !globalThis.__openNextAls?.getStore()?.isISRRevalidation)'`;
export const patchFetchCacheForISR = {
    name: "patch-fetch-cache-for-isr",
    patches: [
        {
            versions: ">=14.0.0",
            field: {
                pathFilter: getCrossPlatformPathRegex(String.raw `(server/chunks/.*\.js|.*\.runtime\..*\.js|patch-fetch\.js)$`, { escape: false }),
...
export const patchUnstableCacheForISR = {
    name: "patch-unstable-cache-for-isr",
    patches: [
        {
            versions: ">=14.2.0",
            field: {
                pathFilter: getCrossPlatformPathRegex(String.raw `(server/chunks/.*\.js|.*\.runtime\..*\.js|spec-extension/unstable-cache\.js)$`, { escape: false }),
                contentFilter: /\.isOnDemandRevalidate/,
...
export const patchUseCacheForISR = {
    name: "patch-use-cache-for-isr",
    patches: [
        {
            versions: ">=15.3.0",
            field: {
                pathFilter: getCrossPlatformPathRegex(String.raw `(server/chunks/.*\.js|\.runtime\..*\.js|use-cache/use-cache-wrapper\.js)$`, { escape: false }),
                contentFilter: /\.isOnDemandRevalidate/,
