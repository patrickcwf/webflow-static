...
function validateFunctionOptions(fnOptions) {
    ...
    const wrapper = typeof fnOptions.override?.wrapper === "string"
        ? fnOptions.override.wrapper
        : "aws-lambda";
    const converter = typeof fnOptions.override?.converter === "string"
        ? fnOptions.override.converter
        : "aws-apigw-v2";
    if (fnOptions.override?.generateDockerfile &&
        converter !== "node" &&
        wrapper !== "node") {
        logger.warn("You've specified generateDockerfile without node converter and wrapper. Without custom converter and wrapper the dockerfile will not work");
    }
    if (converter === "aws-cloudfront" && fnOptions.placement !== "global") {
        logger.warn("You've specified aws-cloudfront converter without global placement. This may not generate the correct output");
    }
    ...
    if (!compatibilityMatrix[wrapper].includes(converter) &&
        !isCustomWrapper &&
        !isCustomConverter) {
        logger.error(`Wrapper ${wrapper} and converter ${converter} are not compatible. For the wrapper ${wrapper} you should only use the following converters: ${compatibilityMatrix[wrapper].join(", ")}`);
    }
}
...
export function validateConfig(config) {
    ...
    if (config.dangerous?.disableTagCache) {
        logger.warn(`You've disabled tag cache.
