 ...
 */
export async function compileOpenNextConfig(baseDir, openNextConfigPath, { nodeExternals = "", compileEdge = false } = {}) {
    const sourcePath = path.join(baseDir, openNextConfigPath ?? "open-next.config.ts");
    const buildDir = fs.mkdtempSync(path.join(os.tmpdir(), "open-next-tmp"));
    let configPath = compileOpenNextConfigNode(sourcePath, buildDir, nodeExternals.split(","));
    // On Windows, we need to use file:// protocol to load the config file using import()
    if (process.platform === "win32")
        configPath = `file://${configPath}`;
    const config = (await import(configPath)).default;
    if (!config || !config.default) {
        logger.error("config.default cannot be empty, it should be at least {}, see more info here: https://opennext.js.org/config#configuration-file");
        process.exit(1);
    }
    ...
    const usesEdgeRuntime = (config.middleware?.external && config.middleware.runtime !== "node") ||
...
export function compileOpenNextConfigNode(sourcePath, outputDir, externals) {
...
export function compileOpenNextConfigEdge(sourcePath, outputDir, externals) {
