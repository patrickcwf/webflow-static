...
export function normalizeOptions(config, distDir, tempBuildDir) {
 ...
 */
export function removeFiles(root, conditionFn) {
...
export function getHtmlPages(dotNextPath) {
    ...
    return Object.entries(JSON.parse(manifest))
        ...
        .reduce((acc, page) => acc.add(page), new Set());
}
export function getBuildId(options) {
    return fs
        .readFileSync(path.join(options.appBuildOutputPath, ".next/BUILD_ID"), "utf-8")
        .trim();
}
 ...
 */
export function checkRunningInsideNextjsApp(options) {
    const { appPath } = options;
    const extension = ["js", "cjs", "mjs", "ts"].find((ext) => fs.existsSync(path.join(appPath, `next.config.${ext}`)));
    if (!extension) {
        logger.error("Error: next.config.js not found. Please make sure you are running this command inside a Next.js app.");
        process.exit(1);
    }
}
export function printNextjsVersion(options) {
 ...
 */
export async function isEdgeRuntime(overrides) {
