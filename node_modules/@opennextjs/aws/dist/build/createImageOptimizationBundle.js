...
export async function createImageOptimizationBundle(options) {
    ...
    await buildHelper.esbuildAsync({
        entryPoints: [
            path.join(options.openNextDistDir, "adapters", "image-optimization-adapter.js"),
        ],
        external: ["sharp", "next"],
    ...
    fs.copyFileSync(path.join(appBuildOutputPath, ".next/required-server-files.json"), path.join(outputPath, ".next/required-server-files.json"));
    ...
    installDependencies(outputPath, config.imageOptimization?.install ?? {
